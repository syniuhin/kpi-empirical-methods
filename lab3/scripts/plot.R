...
f.pdf.plot <- function(fun.df=df.all, n=10000, k1, k2, xmax=5, precision=.1) {
  f.df <- fun.df(n, k1, k2)
  ggplot(f.df, aes(x = xemp, colour = y)) +
    geom_area(alpha = .1, binwidth = precision, position = "identity",
              stat = "bin") +
    coord_cartesian(xlim = c(0, xmax)) +
    ylab('Ймовірність') + xlab('Значення випадкової величини')
}

# The same as above, but instead plotting cdfs of results.
f.cdf.plot <- function(fun.df=df.all, n=10000, k1, k2, xmax=5, precision=.1) {
  f.df <- fun.df(n, k1, k2)
  ggplot(f.df, aes(x = xemp, colour = y)) +
    stat_ecdf(pad = F) + coord_cartesian(xlim = c(0, xmax)) +
    ylab('Плотність') + xlab('Значення випадкової величини')
}